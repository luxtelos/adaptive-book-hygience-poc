...
export class ClaudeAdapter extends BaseLLMService {
  ...
  private async loadAssessmentPrompt(): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to load assessment prompt: ${response.statusText}`);
      }
      this.promptCache = await response.text();
      logger.debug('Assessment prompt loaded and cached for Claude');
      return this.promptCache;
    } catch (error) {
      logger.error('Failed to load hygiene assessment prompt for Claude', error);
      throw error;
    }
  }
  ...
  async analyzeAccountingQuality(rawData: any): Promise<any> {
    logger.info('Starting accounting quality analysis with Claude (large dataset)');
    ...
    try {
      ...
      logger.debug(`Formatted data size: ${formattedData.length} chars, estimated tokens: ${this.estimateTokens(formattedData)}`);
  ...
  private extractBusinessOwnerSection(response: string): any {
    const healthScoreMatch = response.match(/(?:Health|Overall)\s+Score[:\s]*(\d+\/100)/i);
    const healthScore = healthScoreMatch ? healthScoreMatch[1] : '';

    const whatThisMeansMatch = response.match(/WHAT\s+THIS\s+MEANS[:\s]*([\s\S]*?)(?=KEY\s+FINDINGS|RECOMMENDED|##|$)/i);
    const whatThisMeans = whatThisMeansMatch ? whatThisMeansMatch[1].trim() : '';

    const keyFindings: string[] = [];
    const findingsMatch = response.match(/KEY\s+FINDINGS[:\s]*([\s\S]*?)(?=RECOMMENDED|NEXT\s+STEPS|##|$)/i);
    if (findingsMatch) {
    ...
    const stepsMatch = response.match(/(?:RECOMMENDED\s+)?NEXT\s+STEPS[:\s]*([\s\S]*?)(?=##|TECHNICAL|BOOKKEEPER|$)/i);
    if (stepsMatch) {
  ...
  private extractBookkeeperSection(response: string): any {
    ...
    const issuesMatch = response.match(/CRITICAL\s+ISSUES[:\s]*([\s\S]*?)(?=RECOMMENDED\s+IMPROVEMENTS|ONGOING|##|$)/i);
    if (issuesMatch) {
      const priorityMatches = issuesMatch[1].matchAll(/Priority\s+(\d+)[:\s]*([^\n]+)/gi);
      let priority = 1;
    ...
    const improvementsMatch = response.match(/RECOMMENDED\s+IMPROVEMENTS[:\s]*([\s\S]*?)(?=ONGOING|##|$)/i);
    ...
    const maintenanceMatch = response.match(/ONGOING\s+MAINTENANCE[:\s]*([\s\S]*?)(?=##|ASSESSMENT\s+METHODOLOGY|$)/i);
    if (maintenanceMatch) {
